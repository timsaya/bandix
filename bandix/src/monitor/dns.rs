...
impl DnsMonitor {
    ...
    pub async fn start(
        ...
        let mut ringbuf = if let Some(map) = ctx.dns_map.take() {
            ...
            let ingress_ebpf = ctx.ingress_ebpf.take().ok_or_else(|| {
    ...
    async fn process_ringbuf_events(
        &self,
        ringbuf: &mut RingBuf<MapData>,
        ctx: &DnsModuleContext,
    ) {
        ...
        while let Some(item) = ringbuf.next() {
            ...
            if bytes.len() < header_size {
                log::warn!("DNS RingBuf item too small: {} bytes (expected at least {})", bytes.len(), header_size);
                continue;
            }
            ...
            let cap_len = std::cmp::min(
                payload_all.len(),
                header.captured_len as usize
            );
            ...
            match self.parse_dns_packet_from_ethernet(payload, header.timestamp, ctx) {
                ...
                None => {
                    ...
                    // Log it as a warning to help debug
                    if cap_len >= 14 {
                        let eth_type = u16::from_be_bytes([payload[12], payload[13]]);
                        let direction_str = if header.direction == 0 { "Ingress" } else { "Egress" };
                        log::warn!(
                            "Received non-DNS packet [{}] [Length:{}] [Ethernet type:0x{:04X}] - eBPF filtering may have issues",
                            direction_str, cap_len, eth_type
                        );
                    } else {
                        log::warn!(
                            "Received packet but length insufficient [{}] - data may be corrupted",
                            cap_len
                        );
                    }
                }
    ...
    fn parse_dns_packet(
        ...
        if data.len() < dns_offset + 12 {
            return format!(
                ...
                src_ip, src_port, dst_ip, dst_port, data.len(), dns_offset + 12
            );
        }
